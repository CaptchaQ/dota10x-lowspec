...
- **Original authors of vendored mods**:
  - **Egezenn** ([@Egezenn](https://github.com/Egezenn)) — `Minify Spells & Items`, `Minify Base Attacks`, `Remove Hero Renders`, `Remove Showcases`, `Remove Main Menu Background`, `Tree Mod`
...
- **Modifications by this project**:
  - The mods listed above (with the exception of `Simple Dark Terrain`
    and `Remove Tree Models`) are copied verbatim from upstream and used
    as inputs to this project's CLI builder.
  - `Simple Dark Terrain` is a derivative of `Dark Terrain` by robbyz512.
    ...
    derivative work.
  - `Remove Tree Models` is a derivative work using upstream's
    `blacklist.txt` mechanism. It ships only a 312-line blacklist of
    `.vmdl_c` paths (no file payloads); the list was generated from the
    public file index of `pak01_dir.vpk` to cover trees / bushes /
    foliage models that upstream `Remove Foilage` and `Tree Mod` do
    not touch directly. The mod remains GPL-3.0 as a derivative work.
